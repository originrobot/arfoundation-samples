...
namespace Crosstales.RTVoice.Provider
{
    ...
    public class VoiceProviderWindows : BaseVoiceProvider
    {
        ...
        public override bool isWorkingInPlaymode
        {
            get
            {
                return Util.Helper.isWindowsEditor;
            }
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (System.IO.File.Exists(applicationName))
                    {
                        ...
                        bool start = false;
#if ENABLE_IL2CPP
                        System.Threading.Thread worker = new System.Threading.Thread(() => readSpeakNativeStream(ref process, ref speechTextArray, out wordIndex, out phoneme, out viseme, out start, useVisemesAndPhonemesIL2CPP, useVisemesAndPhonemesIL2CPP)) { Name = wrapper.Uid.ToString() };
#else
                        System.Threading.Thread worker = new System.Threading.Thread(() => readSpeakNativeStream(ref process, ref speechTextArray, out wordIndex, out phoneme, out viseme, out start)) { Name = wrapper.Uid.ToString() };
#endif
                        worker.Start();
...
#else
        private void readSpeakNativeStream(ref System.Diagnostics.Process process, ref string[] speechTextArray, out int wordIndex, out string phoneme, out string viseme, out bool start, bool redirectOutputData = true, bool redirectErrorData = true)
#endif
        {
            wordIndex = 0;
            phoneme = string.Empty;
